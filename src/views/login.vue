      ...
      <div class="banner-bg">
        <div class="login-form">
                ...
                <div class="phone-login">
                  <div class="form">
                    ...
                    <div class="code">
                      ...
                      <div class="right-img">
                        <div v-if="!isGetCode" class="code-phone" @click="getCode">{{codeInfo}}</div>
                        <div v-else class="getting-code">重新获取({{codeNum}})</div>
                      </div>
                    </div>
          ...
          <template v-else-if="isCamera == 2">
            <div class="phone-reg phone_mask">
              <!-- <div class="left-reg-mask"></div> -->
              <!-- <div class="right-reg-mask"></div> -->
              <div class="top-title">手机注册</div>
              ...
              <div class="code">
                ...
                <div class="right-img">
                  <div v-if="!isRegGetCode" class="code-phone" @click="getRegCode">{{codeRegInfo}}</div>
                  <div v-else class="getting-code">重新获取({{codeRegNum}})</div>
                </div>
              </div>
...
export default {
  data() {
    return {
      ...
      logoBanner: {
        background:
          "url(" +
          require("../assets/img/login-banner.jpg") +
          ") repeat-x center center"
      },
      // warning图标
      warningIcon: {
        background:
          "url(" + require("../assets/img/warning-icon.png") + ") no-repeat"
      },
      // 默认选中第一个登录
      index: 1,
      // 设定滑动的初始值
  ...
  methods: {
      ...
      this.$api
        ...
        .then(res => {
          if (res.code == 200) {
            ...
            this.$store.dispatch("updateIsPhone", this.phone);
            window.localStorage.setItem("Token", access_token);
            this.$store.dispatch("updateToken", access_token);
            // this.$store.dispatch("updaterefreshToken", refresh_token);
    ...
    getRegCode() {
      ...
      var data = { mobile: this.userRegPhone };
      console.log(data + "我是手机");
      this.$api
        .verificationCodes(data)
        .then(res => {
          console.log(res);
          this.keyw = res.data.key;
    ...
    regSubmit() {
      ...
      if (!this.userRegPhone || !this.userRegPsd || !this.userRegCode) {
        ...
        var data = {
          mobile: this.userRegPhone,
          password: this.userRegPsd,
          verification_code: this.userRegCode,
          verification_key: this.keyw
        };
        console.log(data);
        this.$api
          .users(data)
